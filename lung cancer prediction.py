    ...
    if st.button('Predict 🔮'):
        # Create a dictionary for the input
         input_data = {
            ...
            'CHRONIC DISEASE': yn[chronic_disease],
            'FATIGUE ': yn[fatigue],                 # note trailing space
            'ALLERGY ': yn[allergy],                 # note trailing space
            'WHEEZING': yn[wheezing],
            'ALCOHOL CONSUMING': yn[alcohol_consumption],
            'COUGHING': yn[coughing],
            'SHORTNESS OF BREATH': yn[shortness_of_breath],
            'SWALLOWING DIFFICULTY': yn[swallowing_difficulty],
            'CHEST PAIN': yn[chest_pain],
            'GENDER_M': male                          # dummy column from get_dummies
        }
